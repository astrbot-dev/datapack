...
execute if data storage astrbot:temp modules[0].module{module_type:0} as @e[tag=astrbot.createtable.slot.attack, type=interaction, distance=0..2] if score @s astrbot.createtable_id = curr_createtable astrbot.temp run tag @s add astrbot.createtable.slot.disable
