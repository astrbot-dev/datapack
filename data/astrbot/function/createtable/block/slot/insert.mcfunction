...
execute \
    as @e[tag=astrbot.temp, distance=0..0.9, type=interaction] \
...
execute \
    ...
    as @n[tag=astrbot.temp, distance=0..0.9, type=interaction] \
...
# if try to install illegal module on the front attack slot of a flying bot
execute \
    if function astrbot:createtable/block/slot/insert/is_illegal_module \
    as @n[tag=astrbot.temp, distance=0..0.9, type=interaction] \
    unless score @s astrbot.slot matches 0 \
    as @e[tag=astrbot.createtable.bot.model, distance=0..0.9, type=item_display] if score @s astrbot.createtable_id = curr_createtable astrbot.temp \
...
execute \
    ...
    as @e[tag=astrbot.createtable.bot.model, distance=0..0.9, type=item_display] \
...
execute as @n[tag=astrbot.temp, distance=0..0.9, type=interaction] unless score @s astrbot.slot_status = EMPTY astrbot.slot_status run execute positioned ~ ~0.5 ~ run function astrbot:util/macro/summon_item with storage astrbot:temp arg
data modify entity @n[tag=astrbot.temp, distance=0..0.9, type=item_display] item set from entity @s SelectedItem
item replace entity @s weapon.mainhand with air

# set bot modules data and update bot model
execute as @n[tag=astrbot.temp, distance=0..0.9, type=interaction] run function astrbot:createtable/block/slot/set_bot_modules

#the slot is not empty now
execute as @n[tag=astrbot.temp, distance=0..0.9, type=interaction] run function astrbot:createtable/block/slot/set_slot_to_occupied
