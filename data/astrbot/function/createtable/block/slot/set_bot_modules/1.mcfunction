...
execute if data entity @s item.components."minecraft:custom_data".astrbot.modules[{slot:1}].enabled store result storage astrbot:temp module_info.enabled byte 1.0 run data get entity @s item.components."minecraft:custom_data".astrbot.modules[{slot:1}].enabled
