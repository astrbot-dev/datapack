tag @s add self
execute as @e[tag=astrbot.createtable.slot.attack, tag=!self, distance=0.1..2, type=interaction] if score @s astrbot.createtable_id = curr_createtable astrbot.temp run tag @s add astrbot.createtable.slot.another
tag @s remove self
