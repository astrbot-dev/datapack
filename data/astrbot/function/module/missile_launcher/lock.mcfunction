execute as @s at @s positioned ^ ^ ^64 run scoreboard players set @e[distance=..64,tag=!lock,tag=!agm,type=!item_display,type=!block_display,type=!text_display,type=!item,type=!interaction] astrbot.target 1
execute as @s at @s anchored eyes run function astrbot:module/missile_launcher/ray
