...
execute as @e[tag=uid,limit=1] at @e[tag=launch] at @n[tag=astrbot_heavy,tag=astrbot_missile_launcher] positioned ~ ~1.5 ~ summon minecraft:item_display run tag @s add agmpre
...
execute if entity @e[tag=uid] run scoreboard players remove @n[tag=astrbot_heavy,tag=astrbot_missile_launcher] astrbot.player_power 10
